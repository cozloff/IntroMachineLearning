...
# Continuous real-valued Input Features: carat, depth-percentage, width-percentage, length, width, depth
X = data_set.drop('price', axis=1)  

# Output characteristics: price (US dollars) 
Y = data_set['price']  
...
# Pairing each OLS parameter with its corresponding column name
column_names = X_train.columns.tolist()
equation_parts = [f"{OLS_params[0]}*{column_names[0]}"]
equation_parts.extend([f"{param}*{column}" for param, column in zip(OLS_params[1:], column_names[1:])])

# Join all parts into a single unified equation
equation = " + ".join(equation_parts)

print("OLS Linear Equation:")
...
plt.title('Diamond Price Correlation Matrix')
plt.show()
